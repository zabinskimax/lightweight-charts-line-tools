...
export class PriceRangePaneView extends LineToolPaneView {
	...
	protected override _updateImpl(): void {
		...
		const cullingFirstValue = ownerSource?.firstValue();
		if (!cullingFirstValue) { return; }

		const point0ScreenY = priceScale.priceToCoordinate(point0Data.price, cullingFirstValue.value);
		const point1ScreenY = priceScale.priceToCoordinate(point1Data.price, cullingFirstValue.value);
		...
		if (!isOutsideView || options.priceRange.extend.left || options.priceRange.extend.right) {
			...
			if (options.text.value) {
				...
				this._labelRenderer.setData({ text: labelOptions, points: [pivot], editing: false });
