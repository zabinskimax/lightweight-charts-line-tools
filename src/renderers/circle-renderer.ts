...
export class CircleRenderer implements IPaneRenderer {
    ...
    public draw(ctx: CanvasRenderingContext2D, pixelRatio: number, isHovered: boolean, hitTestData?: unknown): void {
        ...
        this._drawBorder(ctx, centerX, centerY, this._calculateCircleRadius(point0, point1), scaledBorderWidth, borderColor);

        ctx.restore();
    }

    private _getPointsInPhysicalSpace(pixelRatio: number): [Point, Point] {
        ...
        const scaledMaxX = Math.round(maxX * pixelRatio);
        const scaledMinY = Math.round(minY * pixelRatio);
        const scaledMaxY = Math.round(maxY * pixelRatio);
        return [new Point(scaledMinX, scaledMinY), new Point(scaledMaxX, scaledMaxY)];
    }
    ...
    private _drawBorder(ctx: CanvasRenderingContext2D, centerX: number, centerY: number, radius: number, scaledBorderWidth: number, borderColor: string | undefined): void {
        if (borderColor !== undefined && scaledBorderWidth > 0) {
            ctx.beginPath();
